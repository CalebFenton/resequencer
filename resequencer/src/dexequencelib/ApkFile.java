...
public class ApkFile extends java.io.File {
    ...
    public void apkAssemble() throws Exception {
        ...
        if (DecodeResources != DecodeResOption.FULL_DECODE) {
            ...
            File outFile = File.createTempFile("atapk", null);

            // appDirName, outFile, forceBuildAll, debug
            ...
            ExtFile apk = new ExtFile(ApkPath);
            new Androlib().build(new File(DumpDir), outFile, flags, apk, mAaptPath);
    ...
    public void apkDecompile(final String dumpPath) throws Exception {
        ...
        String[] args = new String[] { "--use-locals", "--sequential-labels", "--api-level", AppInfo.AppMinSDK + "",
                        "--output", outDir, ClassesDex.getPath() };
    ...
    private void loadXMLStrings() {
        ...
        String xmlStringsPath = DumpDir + File.separator + "res" + File.separator + "values" + File.separator
                        + "strings.xml";
        File xmlStrings = new File(xmlStringsPath);

        if (!xmlStrings.exists()) {
            xmlStringsPath = DumpDir + File.separator + "res" + File.separator + "values-en" + File.separator
                            + "strings.xml";
            xmlStrings = new File(xmlStringsPath);
        }

        // contains the ID numbers and names
        String xmlPublicPath = DumpDir + File.separator + "res" + File.separator + "values" + File.separator
                        + "public.xml";
